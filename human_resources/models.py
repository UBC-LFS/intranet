...
class HumanResourcesIndex(RoutablePageMixin, Page):
    ...
    def get_context(self, request):
        context = super(HumanResourcesIndex, self).get_context(request)
        context["posts"] = (
            HumanResourcePage.objects.descendant_of(self).live().order_by("-publish_date")
        )
        return context

    def get_posts(self, tag=None):
        posts = HumanResourcePage.objects.live().descendant_of(self)
        if tag:
